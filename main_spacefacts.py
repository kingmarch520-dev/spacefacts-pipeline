...
# Optional: raise early if keys are missing (helps debugging)
...
if PEXELS_API_KEY == "PASTE_YOUR_KEY_HERE":
    print("WARNING: PEXELS_API_KEY not set. Video fetching will fail.")
...
def generate_script(topic: str) -> dict:
    ...
    model = genai.GenerativeModel("gemini-3.6-flash")   # keep your model name
...
def synthesize_scene_audio(scenes: list, run_dir: Path) -> list:
    ...
    for i, scene in enumerate(scenes):
        ...
        # In Colab, asyncio.run() usually works; if you get a loop error,
        # replace with: loop = asyncio.get_event_loop(); loop.run_until_complete(_synthesize(...))
...
# ------------------------------------------------------------------
# 4. ASSEMBLY (moviepy) — keep your vfx usage
# ------------------------------------------------------------------
...
def build_video(script: dict, audio_clips: list, visuals: list, run_dir: Path) -> Path:
    ...
    for i, scene in enumerate(script["scenes"]):
        ...
        # FIXED: pass a tuple for x,y position
        clip = clip.with_effects([vfx.Resize(height=VIDEO_H)]).with_position(("center", "center"))

        # FIXED: added font='Arial' for consistent rendering
        caption = TextClip(
