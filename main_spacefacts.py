...
(CACHE_DIR / "pexels").mkdir(exist_ok=True)
(CACHE_DIR / "pollinations").mkdir(exist_ok=True)
...
def synthesize_scene_audio(scenes: list, run_dir: Path) -> list:
    ...
    # Use get_event_loop for compatibility (similar to v1)
...
def get_cached_visual(scene: dict, index: int, run_dir: Path) -> dict:
    ...
    # Ensure cache subdirs exist (already done at top, but just in case)
    (CACHE_DIR / "pexels").mkdir(exist_ok=True)
    (CACHE_DIR / "pollinations").mkdir(exist_ok=True)
...
def build_video(script: dict, audio_clips: list, visuals: list, run_dir: Path, topic: str) -> Path:
    ...
    try:
        ...
        try:
            ...
            txt = TextClip("Video generation failed", font="Arial", font_size=60, color="white", duration=10).with_position(("center", "center"))
